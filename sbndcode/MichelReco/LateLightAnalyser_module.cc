  ...
  for (const auto& label : fInputLabels) {
    ...
    for (const auto& waveform : *handle) {
      ...
      for (const auto& [muonPeakIndex, michelPeakIndex] : peakPairs) {
        ...
        fMuonFitParam0 = fMuonParams[0]; fMuonFitParam1 = fMuonParams[1];
        ...
        std::vector<double> fMichelParams;
        std::tie(fBinnedWaveformMichel, fMichelParams) = binAndFit(correctedWaveform, michelPeakIndex, fChannel);
        fMichelFitParam0 = fMichelParams[0]; fMichelFitParam1 = fMichelParams[1];
...
std::pair<std::vector<double>, std::vector<double>> sbnd::LateLightAnalyser::binAndFit(const std::vector<double>& data, size_t peakIndex, int channel) {
  auto conversionFactor = (fPDMap.electronicsType(channel) == "daphne") ? fDaphneFreq : fCAENFreq;
