...
private:
  void SelectStopping(const art::Event& e, std::vector<art::Ptr<recob::OpFlash>> &opflashVect);
  ...
  std::string _latelight_producer;
    ...
    : EDProducer{p}
{
  ...
  _latelight_producer = p.get<std::string>("LateLightProducer", "latelightproducer");
...
void SBNDOpT0Finder::SelectStopping(const art::Event &e, std::vector<art::Ptr<recob::OpFlash>> &opflashVect)
{
  // Get the StoppingMuonTrigger objects from the event
  art::Handle<std::vector<sbnd::StoppingMuonTrigger>> stoppingMuonHandle;
  std::vector<art::Ptr<sbnd::StoppingMuonTrigger>> stoppingMuonVect;
  if (e.getByLabel(_latelight_producer, stoppingMuonHandle))
  {
    art::fill_ptr_vector(stoppingMuonVect, stoppingMuonHandle);
  }
  std::cout << "Found " << stoppingMuonHandle->size() << " muon triggers \n";

  // Erase OpFlash objects that do not match the criteria
  opflashVect.erase(
      std::remove_if(
          opflashVect.begin(), opflashVect.end(),
          [&stoppingMuonVect](const art::Ptr<recob::OpFlash> &opflash)
          {
            // Check if this OpFlash time is within 0.02 seconds of any StoppingMuonTrigger MuonTime
            for (const auto &stoppingmu : stoppingMuonVect)
            {
              if (std::fabs(opflash->Time() - stoppingmu->MuonTime) < 0.05)
              {
                std::cout << "  Found match at " << opflash->Time() << "\n";
                return false; // Keep this OpFlash
              }
            }
            return true; // Remove this OpFlash
          }),
      opflashVect.end());
}
                             ...
                             std::unique_ptr<art::Assns<recob::OpFlash, sbn::OpT0Finder>> &flash_opt0_assn_v)
{
  ...
  SelectStopping(e, flash_pmt_v);
  SelectStopping(e, flash_ara_v);
  ...
  for (_matchid = 0; _matchid < (int)(_result_v.size()); ++_matchid)
  {
    ...
    _flash_pe = 0.;
    _hypo_pe = 0.;
    for (auto const &v : _hypo_spec)
      _hypo_pe += v;
    for (auto const &v : _flash_spec)
      _flash_pe += v;
    ...
    for (size_t n_pfp = 0; n_pfp < pfp_v.size(); n_pfp++)
    {
      ...
      if (pfpistrack)
      {
        ...
        for (size_t n_trk = 0; n_trk < track_v.size(); n_trk++)
        {
          ...
          if (abs(trk_start.X()) >= 2.0 * geo->DetHalfWidth() - 3.0)
          {
            exit_pt = trk_start;
            uncontained = true;
          }
          else if (abs(trk_end.X()) >= 2.0 * geo->DetHalfWidth() - 3.0)
          {
            exit_pt = trk_end;
            uncontained = true;
          }
          if (uncontained && _exclude_exiting)
          {
            ...
            for (size_t opch = 0; opch < geo->NOpDets(); opch++)
            {
              ...
              if (_pds_map.isPDType(opch, "pmt_uncoated") || _pds_map.isPDType(opch, "xarapuca_vis"))
                continue;
              if (_use_arapucas && _pds_map.isPDType(opch, "xarapuca_vuv"))
                continue;
          ...
          if (!_track_const_conv)
          {
            ...
            {
              if (calo->PlaneID().Plane == 0)
                calo_v.at(0) = calo;
              if (calo->PlaneID().Plane == 1)
                calo_v.at(1) = calo;
            ...
            if (!_collection_only)
            {
              ...
              bestPlane_trk = ((calo_v[2]->dEdx().size() == maxHits) ? 2 : (calo_v[1]->dEdx().size() == maxHits) ? 1
                                                                       : (calo_v[0]->dEdx().size() == maxHits)   ? 0
                                                                                                                 : -1);
              if (bestPlane_trk == -1)
                continue;
            }
            ...
            for (size_t n_calo = 0; n_calo < calo->dEdx().size(); n_calo++)
            {
              ...
              if ((x_calo < 0 && tpc == 1) || (x_calo > 0 && tpc == 0))
                continue; // skip if not in the correct TPC
          ...
          if (_track_const_conv)
          {
            ...
            for (size_t n_spacepoint = 0; n_spacepoint < spacepoint_v.size(); n_spacepoint++)
            {
              ...
              if (!_collection_only)
              { // find best track planes if other planes are allowed
                int nhit0_trk = 0;
                int nhit1_trk = 0;
                int nhit2_trk = 0;
                for (size_t n_hit = 0; n_hit < hit_v.size(); n_hit++)
                {
                  ...
                  if (hit->View() == 0 && hit->WireID().TPC == tpc)
                    nhit0_trk++;
                  if (hit->View() == 1 && hit->WireID().TPC == tpc)
                    nhit1_trk++;
                  if (hit->View() == 2 && hit->WireID().TPC == tpc)
                    nhit2_trk++;
                }
                const int maxHits = std::max({nhit0_trk, nhit1_trk, nhit2_trk});
                bestPlane_trk = ((nhit2_trk == maxHits) ? 2 : (nhit1_trk == maxHits) ? 1
                                                          : (nhit0_trk == maxHits)   ? 0
                                                                                     : -1);
              }
              for (size_t n_hit = 0; n_hit < hit_v.size(); n_hit++)
      ...
      else if (pfpisshower)
      {
        ...
        for (size_t n_shw = 0; n_shw < shower_v.size(); n_shw++)
        {
          ...
          for (size_t n_spacepoint = 0; n_spacepoint < spacepoint_v.size(); n_spacepoint++)
          {
            ...
            if (!_collection_only)
            { // find best shower plane if other planes are allowed
              ...
              if ((shower->Energy()).at(bestPlane_shw) == -999)
              {
                ...
                bestPlane_shw = ((nhit2_shw == maxHits) ? 2 : (nhit1_shw == maxHits) ? 1
                                                          : (nhit0_shw == maxHits)   ? 0
                                                                                     : -1);
              }
            }
