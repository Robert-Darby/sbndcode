...
private:
  ...
  std::vector<std::vector<short int>> channel_bin_wvfs;
  std::vector<short> wvf_bin_0;
  std::vector<short> paired;
  std::vector<std::vector<short>> unpaired_wvfs;
...
void pmtTriggerProducer::produce(art::Event &e)
{
  ...
  if (i_ev != -1 && i_ev < 4)
  {
    if (fVerbose)
    {
      std::cout << "Outputting Hists" << std::endl;
    }
  }
  ...
  if (fVerbose)
  {
    ...
    for (auto const &opdet : fOpDetsToPlot)
    {
      std::cout << opdet << " ";
    }
    std::cout << std::endl;
  ...
  double fMaxEndTime = -std::numeric_limits<double>::max();  // in us

  for (auto const &wvf : (*waveHandle))
  {
    fChNumber = wvf.ChannelNumber();
    opdetType = pdMap.pdType(fChNumber);
    if (std::find(fOpDetsToPlot.begin(), fOpDetsToPlot.end(), opdetType) == fOpDetsToPlot.end())
    {
      continue;
    }
    if (wvf.TimeStamp() < fMinStartTime)
    {
      fMinStartTime = wvf.TimeStamp();
    }
    if ((double(wvf.size()) / fSampling + wvf.TimeStamp()) > fMaxEndTime)
    {
  ...
  if (fVerbose)
  {
    std::cout << "MinStartTime: " << fMinStartTime << " MaxEndTime: " << fMaxEndTime << std::endl;
  }
  ...
  passed_trigger.reserve(int((fMaxEndTime - fMinStartTime) / (4. / fSampling)));
  ...
  for (double i = fMinStartTime; i < fMaxEndTime + (4. / fSampling); i += (4. / fSampling))
  {
    passed_trigger.push_back(0);
  }

  if (fPair2.size() != paired.size())
  {
    std::cout << "Pair lists mismatched sizes!" << std::endl;
  }
  ...
  for (auto const &wvf : (*waveHandle))
  {
    ...
    {
      if (channel_bin_wvfs.at(i_ch).at(i) == 1 || wvf_bin[i] == 1)
      {
        channel_bin_wvfs.at(i_ch)[i] = 1;
      }
      else
      {
        channel_bin_wvfs.at(i_ch)[i] = 0;
      }
    }
    // std::cout << *max_element(wvf_bin.begin(), wvf_bin.end()) <<  "\n";
    wvf_bin.clear();
  ...
  { // wvf_bin is a vector with entries making up the wvf, one for every channel
    wvf_num++;
    ...
    std::vector<short> wvf_bin_down;
    wvf_bin_down.reserve(int(wvf_bin.size() / 4));
    for (unsigned int i = 0; i < wvf_bin.size(); i++)
    {
      if (i % 4 == 0)
      {
        wvf_bin_down.push_back(wvf_bin[i]);
      }
    }
    ...
    {
      if (fUnpaired.at(i) == (int)fChNumber)
      {
        found = true;
        unpaired = true;
      }
    ...
    if (!found)
    {
      ...
      for (size_t i = 0; i < fPair1.size(); i++)
      {
        if (fPair1.at(i) == (int)fChNumber && paired.at(i) == 1)
        {
          found = true;
          pair_num = i;
          combine = true;
        ...
        else if (fPair1.at(i) == (int)fChNumber && paired.at(i) == 0)
        {
          found = true;
          unpaired_wvfs.at(i) = wvf_bin_down;
          paired.at(i) = 1;
          break;
        }
      }
      if (!found)
      {
        ...
        {
          if (fPair2.at(i) == (int)fChNumber && paired.at(i) == 1)
          {
            found = true;
            pair_num = i;
            combine = true;
          ...
          else if (fPair2.at(i) == (int)fChNumber && paired.at(i) == 0)
          {
            found = true;
            unpaired_wvfs.at(i) = wvf_bin_down;
            paired.at(i) = 1;
    ...
    if (combine || unpaired)
    {
      std::vector<short> wvf_combine;
      wvf_combine.reserve(wvf_bin_down.size());
      if (combine)
      {
        if (unpaired_wvfs.at(pair_num).size() != wvf_bin_down.size())
        {
          std::cout << "Mismatched paired waveform size" << std::endl;
        }
        for (unsigned int i = 0; i < wvf_bin_down.size(); i++)
        {
          if (fPairLogic == "OR")
          {
            if (unpaired_wvfs.at(pair_num)[i] == 1 || wvf_bin_down[i] == 1)
            {
              wvf_combine.push_back(1);
            }
            else
            {
              wvf_combine.push_back(0);
            }
          }
      ...
      unsigned int startbin = std::floor(binspermus * (fMinStartTime - fStartTime));
      unsigned int endbin = std::ceil(binspermus * (fMaxEndTime - fStartTime));
      if (startbin < 0.)
      {
        startbin = 0;
      }
      if (endbin > wvf_combine.size() - 1)
      {
        endbin = wvf_combine.size() - 1;
      }
      ...
      {
        if (wvf_combine.at(i) == 1)
        {
          passed_trigger.at(i_p)++;
        }
        i_p++;
      }
  ...
  if (fVerbose)
  {
    std::cout << "Length of passed trigger: " << pmt_time.numPassed.size() << std::endl;
  }
  if (fVerbose)
  {
    std::cout << "Max number of PMTs passed: " << pmt_time.maxPMTs << std::endl;
  }
